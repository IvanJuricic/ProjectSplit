    ...
    function eventsController($http, $scope, $timeout, $element, $window) {
        ...
        for (var i = 0; i < 7; i++) {
            ...
            if (dateProp.getDay() === 0) {
                vm.dayList.push('Nedjelja');
            }
            else if (dateProp.getDay() === 1) {
                vm.dayList.push('Ponedjeljak');
            }
            else if (dateProp.getDay() === 2) {
                vm.dayList.push('Utorak');
            }
            else if (dateProp.getDay() === 3) {
                vm.dayList.push('Srijeda');
            }
            else if (dateProp.getDay() === 4) {
                vm.dayList.push('Četvrtak');
            }
            else if (dateProp.getDay() === 5) {
                vm.dayList.push('Petak');
            }
            else if (dateProp.getDay() === 6) {
                vm.dayList.push('Subota');
            }
            ...
            if (vm.category !== '') {
                ...
                if (vm.category !== 'Other') {
                    ...
                    for (var i = 0; i < vm.allEvents.length; i++) {
                        var from = vm.allEvents[i].EVENTDATE.split('-');
                        var f = new Date(from[2], from[1] - 1, from[0]);
                        if (vm.allEvents[i].EVENTTYPE !== 'Predstava' && vm.allEvents[i].EVENTTYPE !== 'Glazba' && vm.allEvents[i].EVENTTYPE !== 'Film' && vm.allEvents[i].EVENTTYPE !== 'Other' && vm.allEvents[i].EVENTTYPE !== 'Music' && vm.allEvents[i].EVENTTYPE !== 'Theatre' && vm.allEvents[i].EVENTTYPE !== 'Cinema' && f.getDate() === vm.date.getDate()) {
                            vm.selectedEvents.push(vm.allEvents[i]);
                        }
                    }
                }
                ...
                if (vm.selectedEvents.length === 0) {
                    vm.eventMesseage = 'Žao nam je, ali ne možemo pronaći događaje koji odgovaraju vašem zahtjevu!';
                }
        ...
        vm.sendCustomEvent = function () {
            ...
            var cEventMoreUrl = vm.customEvent_adress;
            var cEvent = [];
            ...
            cEvent.push(cEventMoreUrl);

            $http.post('/hr', cEvent).then(function (res) {
                vm.customEventTime = '';
                vm.customEventName = '';
                vm.customEventDate = '';
